...
export function AgentShell({ agent, children }: { agent: Agent; children: React.ReactNode }) {
  ...
  return (
    <div className="gday-app relative">
      <header className="sticky top-0 z-40 border-b border-teal-900/6 bg-white/80 backdrop-blur-xl">
        <div className="relative mx-auto flex h-14 max-w-6xl items-center justify-between gap-3 px-4 sm:h-16 sm:px-6">
          ...
          <nav className="hidden items-center gap-1 rounded-full border border-teal-900/8 bg-teal-950/[0.03] p-1 md:flex">
            {nav.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ? 'bg-teal-800 text-white shadow-sm shadow-teal-800/25'
          ...
          <div className="flex min-w-0 shrink-0 items-center gap-2.5 sm:gap-3">
            ...
            <div className="flex size-9 items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-cyan-700 text-xs font-semibold text-white shadow-sm shadow-teal-700/20">
              {agent.name
                .split(' ')
                .slice(0, 2)
                .map((part) => part[0])

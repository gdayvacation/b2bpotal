import { dayBoatPlanKey, isNoTransfer, isVirtualVan, totalPassengers, type Booking, type DayBoatPlan, type DayVehiclePlan, type Program } from '@/lib/types'
import {
  bookingPaxOnVan,
  listVanNumbers,
  primaryVan,
...
export function orderedBookingsForSequence(
  ...
  const vanNums = listVanNumbers(vehiclePlan.assignments).filter((van) => !isVirtualVan(van))
